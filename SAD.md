...
### ADR-016 — Regresión visual con test-runner: _story-as-test_ + axe + diff de pixel determinista por entorno

**Contexto:** §7 pide "visual regression: diffs visuales por componente" (Storybook test-runner / Chromatic). El diff de pixel con snapshots commiteados tiene un problema real: el render de fuentes difiere entre el Windows de desarrollo y el Linux de CI, así que un baseline generado en local rompería el CI por anti-aliasing, no por una regresión real. Un servicio hosteado (Chromatic) se descartó al elegir herramienta; Docker local no está disponible.
**Decisión:** la regresión visual se implementa con **`@storybook/test-runner`** ejecutando **cada story en Chromium real**, con tres capas: (1) _smoke test_ —una story que lanza al renderizar rompe el build ("story-as-test", §10.1)—; (2) **axe por story** (motor real, ve el contraste que jsdom no); (3) **diff de pixel** (`jest-image-snapshot`, comparación SSIM). El determinismo del pixel se resuelve **fijando el entorno de render**: baselines y verificación se generan en el **mismo runner de CI (`ubuntu-24.04`)** con la versión de Chromium fijada por el lockfile. Los baselines se siembran/actualizan con un workflow manual (`visual-snapshots.yml`) que los **comitea** desde ese runner; en local (Windows) el snapshot se **desactiva por env** (`VISUAL_SNAPSHOTS`), corriendo solo smoke + axe.
